...
export default function AssessmentPermitApplicationsPage({ appNo }: { appNo?: string }) {
  ...
  const breadcrumb = useMemo(() => [
    ...
    {
      label: 'Pending Payments for Assessment of Documents List',
    ...
    <>
      ...
      <div className="p-6">
        <CardContainer title="Pending Payments for Assessment of Documents List">
          <PermitManagementTable viewable={true} searchParam={appNo} permitUrl={'/' + role + '/api/permits/payment'} role={session!.user.role} step={8} refreshNotification={refreshNotification} />
        </CardContainer>
      </div>
    </>
  )
