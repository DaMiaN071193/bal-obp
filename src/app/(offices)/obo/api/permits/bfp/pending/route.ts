  ...
  try {
    ...
    if (!!session) {
      const data = await Application.find({
        ...
        $where: 'this.status.length > 4 && (this.status[this.status.length - 1].step === 6 || this.status[this.status.length - 1].step === 7) && this.status[this.status.length - 1].statusType === "' + StatusType.Pending + '"'
