...
export default function AssessmentPermitApplicationsPage({ appNo }: { appNo?: string }) {
  ...
  const breadcrumb = useMemo(() => [
    ...
    {
      label: 'Assessment of Documents',
      url: '/' + role + '/permit/assessment',
    }
  ], [role])
    ...
    <>
      ...
      <div className="p-6">
        <CardContainer title="Assessment of Documents Permit Applications">
          <PermitManagementTable viewable={true} printable={true} editable={true} searchParam={appNo} permitUrl={'/' + role + '/api/permits/assessment'} role={session!.user.role} step={3} refreshNotification={refreshNotification} />
        </CardContainer>
      </div>
    </>
  )
