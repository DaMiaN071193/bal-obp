...
export default function SanitaryPermitForm({ step, draftData }: { step: number, draftData: any; }) {
  ...
  const [scopeOfWork, setScopeOfWork] = useState<SanitaryScopeOfWork|string|undefined>(step === 6 ? draftData?.electricalPermit?.box1?.scopeOfWork?.name : draftData?.sanitaryPermit?.box1?.scopeOfWork?.name)
  const [scopeOfWorkSpecify, setScopeOfWorkSpecify] = useState<string|undefined>(step === 6 ? draftData?.electricalPermit?.box1?.scopeOfWork?.specify : draftData?.sanitaryPermit?.box1?.scopeOfWork?.specify)
  const [useOrCharacterOfOccupancy, setUseOrCharacterOfOccupancy] = useState<string|undefined>(step === 6 ? draftData?.electricalPermit?.box1?.useOrCharacterOfOccupancy : draftData?.sanitaryPermit?.box1?.useOrCharacterOfOccupancy)
        ...
        <div className="flex flex-col md:flex-row md:justify-evenly md:gap-x-4">
          <div className="w-full px-4 pt-4 mb-4 border shadow border-gray-200 bg-lime-50 rounded-md md:ml-2">
            ...
            <TextInputField
              ...
              readOnly={step === 6 || !useNotMyBuildingOwner}
              required
            />
            <TextInputField
              label="Address"
