...
import { NotificationDocument, UserRoles } from './models/interfaces';
...
export async function generateSessionPayload(role: UserRoles, userId: string, expHours: number = 8) {
  await connectDB();
  try {
...
export async function createSession(role: UserRoles, userId: string, expHours: number = 8): Promise<boolean> {
...
export async function getSession(role: UserRoles): Promise<JWTPayload | SessionPayload | { [key: string]: any } | null> {
...
export async function destroySession(role: UserRoles) {
...
export async function updateSession(role: UserRoles): Promise<boolean> {
...
export async function getMyNotifications(role: UserRoles, unreadOnly?: boolean): Promise<NotificationDocument[]|null> {
  await connectDB();
  try {
    const session = await getSession(role);
    if (!session) {
